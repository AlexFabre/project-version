...
usage() {
    ...
    echo "        -f zephyr : Generate a zephyr VERSION file."
...
if [ "$OUTPUT_FORMAT" = "zephyr" ]; then
    ...
    {   echo "# This file declares the firmware revision information";
        ...
        echo "set(APP_FW_NB_COMMITS_SINCE_LAST_TAG $NB_COMMIT_SINCE_LAST_TAG)";
        echo "set(APP_FW_COMMIT_SHORT_SHA          \"$COMMIT_SHA\")";
        echo ""
        echo "# Build date time (UTC)";
        echo "set(APP_FW_BUILD_DAY                 $DAY)";
        echo "set(APP_FW_BUILD_MONTH               $MONTH)";
        echo "set(APP_FW_BUILD_YEAR                $YEAR)";
        echo "set(APP_FW_BUILD_HOUR                $HOUR)";
